			...
			<div class="col-md-10">
				...
				<div class="box box-success">
		          <div class="box-header with-border">
		            <label class="box-title"><b> Edit Agent </b></label>
		          </div>
		          <div class="box-body">
...
<script type="text/javascript">
	...
	function details()
	{
        ...
        $.ajax({
                    url: '<?= base_url() ?>Admin/agent_details',
                    type: 'POST',
                    dataType: 'json',
                    data: data,
                    beforeSend: function() {
                    ...
                    success: function(response){
                    	...
                    	$("#am_title").val(response.title_salutation);
						$("#am_fname").val(response.first_name);
						$("#am_lname").val(response.last_name);
						$("#am_group").val(response.group_name);
						$("#am_role").val(response.role_name);
						$("#am_uname").val(response.username);
						$("#am_pwd").val(response.password);
						$("#am_email").val(response.email);
						$("#am_mobile").val(response.mobile);
						$("#am_valid").val(response.validity);

						$("#old_pwd").val(response.password);
	...
	function submit()
	{
		...
		// check to submit
		if((userid=='')||(am_fname=='')||(am_lname=='')||(am_group=='')||(am_role=='')||(am_uname=='')||(am_pwd=='')||(am_email=='')||(am_mobile=='')||(am_valid=='')){
			...
			var data = 
	            ...
	            $.ajax({
	                    url: '<?= base_url() ?>Admin/edit_agent_submit',
	                    type: 'POST',
	                    dataType: 'html',
	                    data: data,
	                    beforeSend: function() {
