			...
			<div class="col-md-10">
				...
				<div class="box box-success">
		          <div class="box-header with-border">
		            <h3 class="box-title"> <b>Edit Service</b> </h3>
		          </div>
		          <div class="box-body">
					       	...
					       	<!-- SECOND -->
					        	<div class="panel-group">
								  <div class="panel panel-default">
								    <div class="panel-heading">
								      <h4 class="panel-title">
								        <a data-toggle="collapse" href="#collapse1"><i class="fa fa-caret-down"></i> Edit SLA</a>
								      </h4>
								    </div>
	...
	function details()
	{
        ...
        $.ajax({
                    ...
                    success: function(response){
				 		...
				 		var sm_service = $("#sm_service").val(response.service);
						var sm_sub_services = $("#sm_sub_services").val(response.sub_service);
						var sm_type = $("#sm_type").val(response.service_type);
						var sm_critical = $("#sm_critical").val(response.criticalty);
						var sm_valid = $("#sm_valid").val(response.validity);
						var sm_comment = $("#sm_comment").val(response.comment);
                    }
              });
	}
	...
	function submit()
	{
		...
		if(sm_service==''){
			...
			var data = 
	            ...
	            $.ajax({
	                    url: '<?= base_url() ?>Admin/sm_update_service',
	                    type: 'POST',
	                    dataType: 'html',
	                    data: data,
	                    beforeSend: function() {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        var service_id = "<?= $this->uri->segment(3)?>";
        ...
        var addIcon = function ( data, type, row ) {
            if ( type === 'display' ) {
                return '<input type="checkbox" value="'+data+'" class="SLA_ID" name="SLA_ID"> ';
            }
            return data;
        };
    ...
    function check_list_sla()
    {
    	...
    	var data =  {
                ...
                $.ajax({
                        url: '<?= base_url() ?>Admin/check_list_sla',
                        type: 'POST',
                        dataType: 'json',
                        data: data,
                        beforeSend: function() {
                        ...
                        success: function(response){
								...
								var inputElements = document.getElementsByClassName('SLA_ID');
								for(var i=0; inputElements[i]; ++i){

								      cbox = inputElements[i].value;
								      //alert(checkedValue);
								      $.each(response, function(index) {
		        							var sla_id = response[index].sla_id;
											if(sla_id===cbox){
												//alert("True");
												$("input:checkbox[value="+sla_id+"]").attr("checked", true);
											} else {
												//alert("False");
											}
		        					  });
