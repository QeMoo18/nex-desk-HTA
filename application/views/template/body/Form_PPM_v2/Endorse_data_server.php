...
<div class="panel">
    ...
    <div class="panel-body">
            ...
            <div class="col-md-12" style="padding-left: 0px;">
                <a href="<?= base_url()?>Form_PPM/data_server/<?= $this->uri->segment(3)?>"><span class="box2 red2"><i class="fa fa-angle-double-left"></i> Asset </span></a>  
            ...
            <div class="" style="padding-top: 30px; float: right; padding-bottom: 30px;padding-right: 30px;">
                <!-- <div class="col-md-6" style="padding-bottom: 10px;">
                    <a href="<?= base_url()?>Form_PPM/data_server/<?= $this->uri->segment(3)?>">
                        <button class="btn btn-primary">Back To Asset</button>
                    </a>
                </div> -->
                <div class="col-md-6" style="padding-bottom: 10px;">
                    <select class="form-control" name="email_selected">
...
<script type="text/javascript">
    function send_email()
    {
        var email_selected = $("select[name='email_selected']").val();
        var ppm_id = "<?= $this->session->userdata('id_activity');?>";
        
        // var hostname =$("input[name='tick_email[]']").is(":checked");
        // console.log(hostname);


        var cboxes = document.getElementsByName('tick_email[]');
        ...
        for (var i=0; i<len; i++) {
            ...
            if(cboxes[i].checked){
                ...
                // send email 
                var hostname_selected = cboxes[i].value;
                //var user_find = $("#user_find").val();
                hostname.push(hostname_selected);
            } 
        }
        ...
        if(email_selected==''){
          ...
          if(hostname.length>0){
              ...
              $.ajax({
                      url: '<?= base_url() ?>Form_PPM/Server_Send_Email',
                      type: 'POST',
                      dataType: 'html',
                      data: data,
                      beforeSend: function() {
...
<script type='text/javascript'>
   $(document).ready(function(){
    ...
    function loadPagination(pagno){
        ...
        $.ajax({
         data : data,
         url: '<?=base_url()?>Form_PPM/work_server_list_data/'+pagno,
         type: 'post',
         dataType: 'json',
         success: function(response){
     ...
     function get_url(hostname,sno,type,type_ppm_activity)
     {
        ...
        //description
          $.ajax({
             ...
             dataType: 'html',
             success: function(description){
                $("#url_"+sno).html('<input type="checkbox" name="tick_email[]" value="'+hostname+'"></label> '+description);
                // var description = response;
                // $("#description").val(response);
                // var description = $("#description").val();
                // console.log(description);
