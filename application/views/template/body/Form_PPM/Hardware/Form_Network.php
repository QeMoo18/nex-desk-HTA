				...
				<div class="col-md-12">
					...
					<div class="box box-success">
			          <div class="box-header with-border">
			            <label class="box-title"> <b><?= get_name_activity($this->session->userdata('id_activity'))?> : <?= hex2bin($_GET['hostname'])?></b> </label>
			          </div>
			          	<form action="<?=base_url()?>Form_PPM/Add_load_balance" method="post" id="form_data">
				          ...
				          <div class="box-body">
				          		...
				          		<div class="row" style="padding-left: 20px; padding-top: 30px; padding-right: 20px;">
				          			<div class="col-md-6" style="background-color: #fff; border-radius: 10px; padding-top: 10px; border: 1px solid #bdbdbd;">
				          					...
				          					<legend style="font-size: 15px; font-weight: 700;">Load Balance</legend>
									...
									<div class="col-md-5" style="border-radius: 10px; padding-top: 10px;">
											...
											<?php } else { ?>
												...
												<?php } else { ?>
						          					<div class="row">
											            <div class="form-group col-md-12 pull-right">
											              <label for="exampleInputEmail1">Sent to Verify with</label>
											              <input type='text' class='form-control' name='customer' id='customer' list="list_acknowledge">
											            </div>
											            <div class="form-group col-md-12 pull-right" id="btn_submit_div">
											              <button class="btn btn-primary btn-block">Submit to Verify</button>
											            </div>
											        </div>
											    <?php } ?>
...
<script type="text/javascript">
	...
	function detail()
	{
	    ...
	    $.ajax({
	                ...
	                success: function(response){
	                	...
	                	// $("select[name='load_balance_1']").val(response.load_balance_1);
	                	// $("select[name='load_balance_2']").val(response.load_balance_2);
	                	// $("select[name='load_balance_3']").val(response.load_balance_3);


	                	$("[name=load_balance_1]").val([response.load_balance_1]);
	                	$("[name=load_balance_2]").val([response.load_balance_2]);
	                	$("[name=load_balance_3]").val([response.load_balance_3]);
	                	$("[name=hard_disk_usage]").val([response.hard_disk_usage]);
	...
	function disabled_item()
	{
    	...
    	$("[name=load_balance_1]").prop('disabled',true);
    	$("[name=load_balance_2]").prop('disabled',true);
    	$("[name=load_balance_3]").prop('disabled',true);
    	$("[name=hard_disk_usage]").prop('disabled',true);
...
<script type="text/javascript">
		    ...
		    $.ajax({
		                ...
		                success: function(response){
		                	if(response)
		                	{
		                		...
		                		detail();
								$('#form_data').attr('action', '<?= base_url()?>Form_PPM/Update_Load_Balance');
		                	}
		               	}
		          })
		}
	...
	$(document).ready(function (){
		var type_device = "<?= hex2bin($_GET['type'])?>";
		var url = "<?= $_GET['filter']?>";

		if(url=='Search'){

			if(type_device=='Access Point'){
		      window.location.href="<?=base_url()?>Form_PPM/Access_point?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else if(type_device=='Firewall'){
		      window.location.href="<?=base_url()?>Form_PPM/Firewall?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else if(type_device=='Load Balance'){
		      window.location.href="<?=base_url()?>Form_PPM/Load_balance?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else if(type_device=='Switch'){
		      window.location.href="<?=base_url()?>Form_PPM/Switch_PPM?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else if(type_device=='UPS'){
		      window.location.href="<?=base_url()?>Form_PPM/UPS?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else if(type_device=='Controller'){
		      window.location.href="<?=base_url()?>Form_PPM/Controller?hostname=<?= $_GET["hostname"]?>&ppm_id=<?= $_GET["ppm_id"]?>&type=<?= $_GET["type"]?>";
		    } else {
